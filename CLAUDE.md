...
`mode` (`'planning' | 'review' | 'dashboard' | 'admin'`, `MODES` array is the source of truth for the valid values and their order) is the top-level navigation axis, switched via the topbar's mode tabs (`#tabPlanning`/`#tabReview`/`#tabDashboard`/`#tabAdmin`) and `setMode(m)`. Each mode owns one direct child of `<body>`: `#planningBody`/`#reviewBody` (`.body-row` — sidebar + main, same flex-row shape as the original single-mode app) and `#dashboardBody`/`#adminBody` (`.main` directly — a single scrollable panel, no sidebar). `render()` is the single top-level dispatcher: it toggles which of the four containers is visible (`style.display = mode === X ? '' : 'none'` — reverting to `''` rather than hardcoding `'flex'`/`'block'` so the element's CSS class decides its own display value) and calls exactly one of `renderSidebar()+renderMain()` (planning), `renderReview()`, `renderDashboard()`, `renderAdmin()`.
...
Planning mode additionally keeps its own Status/Timeline sub-toggle (`#tabStatus`/`#tabTimeline`, `setView()`), now rendered as a small `.planning-toolbar` above `#main` rather than in the main topbar — it moved there to make room for the mode tabs, which took over the topbar slot it used to occupy.
...
A review cycle is scoped to exactly one workstream (`activeReviewCycle(wsId)` finds the one cycle for that workstream with both `completedAt` and `cancelledAt` still `null` — `startReviewCycle()` is a no-op if one's already active, so there's never more than one open cycle per workstream at a time). The Review sidebar (`renderReviewSidebar()`, `#reviewWsList`) lists every workstream with a small badge: amber "In review" if it has an active cycle, red "Overdue" if `isReviewOverdue(w.id)` (no completed cycle in the last `REVIEW_STALE_DAYS` (30) days, or never reviewed at all), otherwise no badge. Clicking a row calls `setReviewWorkstream(id)`, which persists `reviewWsId` and re-renders just the Review pane.
...
`renderReviewMain()` renders, in order: the workstream header, either a "Start review cycle" button (no active cycle) or the live checklist (`reviewItemRowHtml()` per item — name, dates, its own status badge that opens `openItemModal()` for inline edit-then-confirm, and a confirm toggle button) plus a progress bar and Complete/Cancel buttons (active cycle), then a "History" section listing every `completedAt`/`cancelledAt` cycle for that workstream newest-first (`reviewHistoryRowHtml()`), regardless of how the active-cycle section above rendered.

## Dashboard mode

`renderDashboard()` is read-only — nothing on it mutates state, it only computes and displays. Three cards up top: **Item Status** (total item count + a `.dash-bar` stacked bar of RAG counts, colored via the same `--stat-*` variables as everywhere else, plus a text legend), **Milestone Completion** (`% complete` across every item's milestones, via `allMilestonesFlat()` — a flattened `{...milestone, itemName, workstreamId}` list built fresh each render, not cached), and **Overdue for Review** (count + names of workstreams where `isReviewOverdue()` is true — the same helper Review mode's sidebar badge uses, shared rather than duplicated). Below that: a per-workstream summary table (item count + milestone completion % per workstream), then two side-by-side attention-needed feeds — **Overdue** (items/milestones with `status !== 'complete'` and `dueDate` before today) and **Upcoming** (same, but `dueDate` within the next 7 days) — each entry sorted by date and, for a milestone, labeled `"<item name> — <milestone name>"` so it's clear which item it belongs to without a nested list.

There's no persisted dashboard state and no separate "refresh" action — every value is recomputed from `items`/`workstreams`/`reviewCycles` at render time, so it's automatically current whenever `render()` calls it (i.e. every time `setMode('dashboard')` runs, or any mutation happens while already in Dashboard mode).
