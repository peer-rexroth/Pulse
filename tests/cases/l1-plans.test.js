...
// ---------- L1 Plans no longer has a Dashboard sub-tab — it existed for a
// while (a status breakdown + Delayed list + per-plan rollup table),
// removed entirely on a later, explicit user request ("remove L1
// dashboard"). Every test that exercised l1PlansTab/setL1PlansTab()/
// renderL1PlansDashboardHtml()/l1PlanRollupRows()/l1DelayedEntries()/
// openL1PlanFromDashboard()/openL1MilestoneFromDashboard() is gone along
// with them. isWorkstreamMilestoneLate()/isL1MilestoneActualLate() survive
// below — both are still used by l1MilestoneRowsHtml()/l1LinkedRowHtml()'s
// own row coloring, independent of the removed Dashboard.
