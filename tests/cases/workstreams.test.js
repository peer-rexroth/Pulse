...
test('a workstream row stays active in the sidebar when it is the current filter', function () {
  const w = workstreams[0];
...
test('"All Workstreams" stays active in the sidebar when nothing is filtered', function () {
  filterWorkstreamId = null;
  renderSidebar();
  assertIncludes(document.getElementById('topNavList').innerHTML, 'ws-row-all active', '"All Workstreams" is active while nothing is filtered');
});
