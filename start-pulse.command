#!/bin/bash
# Launches Pulse on a local server (needed for PWA install/offline support,
...
LOG_FILE="$DIR/.pulse-server.log"
URL="http://127.0.0.1:$PORT/pulse.html"
...
if ! is_up; then
  ...
  if [ -f "$PID_FILE" ]; then
    echo "Port $PORT looks busy with something other than Pulse. Edit PORT in this script to change it."
  else
    nohup python3 -m http.server "$PORT" --bind 127.0.0.1 > "$LOG_FILE" 2>&1 &
    echo $! > "$PID_FILE"
    echo "Started Pulse server on $URL (pid $(cat "$PID_FILE"))"
    sleep 0.6
  fi
fi
